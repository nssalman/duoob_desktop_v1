...
class TaskProvider with ChangeNotifier {
  ...
  Future<void> fetchUsersAndTasks() async {
    try {
      ...
      UserProfileModel? userProfileModel = await userRepository.getUserProfileLocal();
      ...
      if (response != null && (response == 'true' || response == 'false')) {
        ...
        if (userTaskRightsResponse != null) {
          ...
          List<TaskRightsModel> taskRightsList = taskRightsModelFromJson(rightsJson);
  ...
  try {
    ...
    if (rightsResponse != null) {
      List<TaskRightsModel> taskRightsList = taskRightsModelFromJson(json.encode(rightsResponse));
      ...
      for (var right in taskRightsList) {
        ...
        if (taskRes != null) {
          ...
          for (var t in tasks) { t.sendToUserId = right.taskOf; }
  ...
  List<TaskModel> addToTaskList({required List<TaskModel> tasks, required List<TaskModel> newTasks, required bool prioritize}) {
    prioritize ? tasks.insertAll(0, newTasks) : tasks.addAll(newTasks);
    return tasks;
  }

  Future<void> addToCustomerTaskList({required List<TaskModel> tasks, required List<TaskModel> newTasks, required bool prioritize}) async {
    prioritize ? myTaskList.insertAll(0, newTasks) : myTaskList.addAll(newTasks);
  }

  Future<void> addToHrTaskList({required List<TaskModel> tasks, required List<TaskModel> newTasks, required bool prioritize}) async {
    prioritize ? hrTaskList.insertAll(0, newTasks) : hrTaskList.addAll(newTasks);
    hrTaskList = hrTaskList.toSet().toList();
  }
 ...
 Future<void> getd365TaskList() async {
  ...
  try {
    ...
    var response = await ApiServices.execute(
      method: apiMethod.get,
      accessToken: token,
      url: '${Constants.apiGetD365Task}?usersid=binu',
    );
...
void selectAll() {
  for (var element in d365TaskList) { element.isSelected = true; }
  isAllSelect = true;
  notifyListeners();
}

void clearSelection() {
  for (var element in d365TaskList) { element.isSelected = false; }
  isAllSelect = false;
  notifyListeners();
}
